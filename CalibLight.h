...
public:
    ...
    void readLabel(string path) {
       /// std::cout<<"/home/sensetime/Downloads/GOD/label_157_part.txt"<<endl
        ...
        while (!fin.eof()) {
            ...
            label.position = cv::Rect(stoi(px), stoi(py), stoi(qx) - stoi(px),
                                      stoi(qy) - stoi(py));
            label.color = type;
    ...
    void readTracjectory(string tPath) {
        ...
        while (!fin.eof()) {
            string line;
            getline(fin, line);
            if (sscanf(line.c_str(), "%f %f %f %f %f %f %f %f %f %f %f %f", &linedata[0],
                       &linedata[1], &linedata[2], &linedata[3], &linedata[4], &linedata[5],
    ...
    ParameterReader() {}
    explicit ParameterReader(string pFile) {
